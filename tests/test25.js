...
setTimeout(() => {
  ...
  ['Overview','Reports','Accounts'].forEach(t => {
  ...
  if (!go) { fail('Overview is not reachable'); }
    ...
    setTimeout(() => {
      ...
      /* Batch management belongs on the board that lists the batches. */
      if (!trainer.document.querySelector('[data-st="newbatch"]'))
        fail('a batch cannot be created from Overview');
      if (!trainer.document.querySelector('[data-st="edit"]'))
        fail('batch details cannot be edited');
      if (!trainer.document.querySelector('[data-st="people"]'))
        fail('batch membership cannot be reached from Overview');
      /* Another trainer's row is numbers only: no action buttons on it. */
      const rows = Array.from(trainer.document.querySelectorAll('.st-table tbody tr'));
      const foreign = rows.filter(r => /Batch 2/.test(r.textContent))[0];
      if (foreign && foreign.querySelector('[data-st="edit"]'))
        fail('another trainer\'s batch offers an Edit button that would be refused');
      done();
    }, 250);
  }
  if (!go) done();
