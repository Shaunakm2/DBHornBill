...
/* Persistence lives outside app.js now. The suites test the desk, not the
   network, so they run against an in-memory double. The real policies are
   tested in tests/01_policies.sql against a live Postgres. */
const STUB = fs.readFileSync(path.join(__dirname, 'store-stub.js'), 'utf8');
