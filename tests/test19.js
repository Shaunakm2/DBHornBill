...
/* Closing a submission now goes through the database and resolves on a later
   tick, so the audit entry is not there the instant the dialog shuts. These
   outcomes are checked after the queue drains, at the bottom of the file. */
...
setTimeout(() => {
  ...
  if (live.length >= 2) {
    ['Client Declined', 'Candidate Declined'].forEach((outcome, i) => {
      ...
      else pendingOutcomes.push(outcome);
    });
  }
  ...
  setTimeout(() => {
    pendingOutcomes.forEach(outcome => {
      nav('audit');
